...
create or replace function public.get_discover_catalog(
  ...
  p_cursor_value timestamptz default null,
...
declare
  ...
  result jsonb;
begin
  if p_sort is null or p_sort not in ('newest','upcoming') then
    raise exception 'Geçersiz sıralama (sort)' using errcode='22023';
  end if;
  if p_period is null or p_period not in ('all','today','week','month') then
    raise exception 'Geçersiz tarih filtresi' using errcode='22023';
  end if;
  if (p_cursor_value is null) <> (p_cursor_id is null)
  ...
  -- The existing invoker RPC intersects dates BEFORE choosing one active
  -- occurrence per event. Its explicit lifecycle predicates also protect
  -- admin callers, whose RLS permits more rows than anonymous callers.
  with active as materialized (
    select r.* from public.list_active_discover_events(range_start,range_end) r
    where r.created_at <= watermark
  ), searched as materialized (
    select a.* from active a
    where needle='' or strpos(public.normalize_discover_search(concat_ws(' ',
      a.title,a.city,a.district,a.venue_name,a.organizer,
      case a.category
        when 'exhibition' then 'Sergi' when 'museum' then 'Müze'
        when 'festival' then 'Festival' when 'fair' then 'Fuar'
        when 'theatre' then 'Tiyatro' when 'concert' then 'Konser'
        when 'workshop' then 'Atölye' when 'university' then 'Üniversite etkinliği'
        when 'city_route' then 'Şehir rotası' when 'day_trip' then 'Günübirlik gezi'
        else a.category end)),needle)>0
    ...
    from local_matches m
    where (nullif(p_category,'') is null or m.category=p_category)
      and (not coalesce(p_free,false) or m.is_free)
      and (not coalesce(p_discount,false) or m.has_student_discount)
    ...
    order by
      case when p_sort='newest' then f.sort_value end desc,
      case when p_sort='newest' then f.id end desc,
      case when p_sort='upcoming' then f.sort_value end asc,
      case when p_sort='upcoming' then f.id end asc
    limit 25
  ), numbered as (
    select c.*,row_number() over(order by
      case when p_sort='newest' then c.sort_value end desc,
      case when p_sort='newest' then c.id end desc,
      case when p_sort='upcoming' then c.sort_value end asc,
      case when p_sort='upcoming' then c.id end asc) as position
    from candidates c
  ...
  select jsonb_build_object(
    ...
    'facets',jsonb_build_object(
      ...
      'categories',coalesce((select jsonb_object_agg(category,amount) from (
        select category,count(*) as amount from local_matches
        where (not coalesce(p_free,false) or is_free) and (not coalesce(p_discount,false) or has_student_discount)
        group by category) categories),'{}'::jsonb),
      'free',(select count(*) from local_matches where is_free
