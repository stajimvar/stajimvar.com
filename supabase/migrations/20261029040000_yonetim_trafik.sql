-- TRAFİK ÖZETİ — GERÇEK VERİ
--
-- Demo üretecinin döndürdüğü şeklin aynısı dönüyor ki panel tarafında
-- yalnız kaynak değişsin. Farklar, uydurulamayacak olanlarda:
--
--   - BOUNCE gerçek tanımıyla hesaplanıyor: tek sayfa görüp ayrılan
--     oturumun oranı. Demo bunu sabit bir aralıktan üretiyordu.
--   - ORTALAMA OTURUM, oturumun ilk ve son olayı arasındaki süre. Tek
--     olaylık oturumlar süreye KATILMIYOR: süreleri sıfır değil,
--     ölçülemez. Sıfır saymak ortalamayı yapay olarak aşağı çekerdi.
--   - HUNİ gerçek sayfa yollarından çıkıyor; 'basvuru' adımı başvuru
--     düğmesine basıldığında yazılan olaydan geliyor.
--
-- Veri yoksa alanlar boş dizi ve sıfır dönüyor. Panel bunu "henüz veri
-- yok" diye gösteriyor; sıfır burada dürüst bir cevap.
...
begin
  ...
  select jsonb_build_object(
    ...
    -- Tek sayfa görüp ayrılanın oranı. Hiç oturum yoksa 0.
    'bounce', (
    ...
    -- ölçülemez. Ölçülebilir oturum yoksa 0.
    'ortalamaOturum', (
    ...
    -- Kaynak, şehir ve cihaz KİŞİ sayıyor: oturum başına bir kez.
    'kaynaklar', (
    ...
    -- Sayfa listesi BAKIŞ sayıyor, kişi değil: aynı kişi bir sayfaya iki
    -- kez bakmışsa iki bakış. Yüzdeler bu yüzden görüntülemeye bölünüyor.
    'sayfalar', (
      select coalesce(jsonb_agg(jsonb_build_object('ad', ad, 'adet', adet)
               order by adet desc), '[]'::jsonb)
    ...
    'huni', (
      select jsonb_build_array(
        ...
        jsonb_build_object('ad', 'İlan listesine baktı',
          'adet', (select count(distinct oturum) from bakis where yol like '/ilanlar%')),
...
comment on function public.yonetim_trafik(text) is
  'Dönemlik trafik özeti (bugun/yedi/otuz). Yalnız yönetici.';
