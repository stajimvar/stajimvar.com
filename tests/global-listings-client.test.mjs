...
test('global katalog siniri exact RPC parametrelerini ve cursor hassasiyetini korur', async () => {
  const calls=[];
  const client={rpc:async(name,args)=>{calls.push({name,args});return {data:{listings:[],facets:{countries:[]},hasMore:false,nextCursor:null,snapshot:'2026-09-05T10:00:00.123456+00:00'},error:null}}};
  ...
  assert.deepEqual(calls,[{name:'get_published_listings_catalog',args:{p_country:'FR',p_cursor_posted_at:'2026-09-01T10:00:00.654321+00:00',p_cursor_id:'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa',p_snapshot:'2026-09-05T10:00:00.123456+00:00'}}]);
