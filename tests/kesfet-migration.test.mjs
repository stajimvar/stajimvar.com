...
try {
  sql = (await Promise.all([
    readFile(new URL('../supabase/migrations/20260827010000_kesfet_events.sql', import.meta.url), 'utf8'),
    readFile(new URL('../supabase/migrations/20260827020000_kesfet_curation.sql', import.meta.url), 'utf8'),
  ])).join('\n');
} catch {}
...
test('curation migration doğrulama, puan ve öğrenci hedefleme alanlarını ekler', () => {
  for (const column of ['last_verified_at','source_trust_score','verification_status','target_audiences','interest_tags','student_fit_score','application_deadline','discount_terms','age_limit','registration_required']) {
