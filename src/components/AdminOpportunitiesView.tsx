...
/*
  KISIT ALANLARI FORMDA HİÇ YOKTU

  Kök neden ölçüldü: `asPayload` bu alanları zaten gönderiyordu ve
  `blank` içinde boş dizi olarak duruyorlardı, ama formda onları
  DOLDURACAK hiçbir girdi yoktu. Yani 68 kaydın tamamı boş kaydedildi;
  parser hatası değil, eksik ekran.

  BOŞ BIRAKMAK BİR DEĞER

  Şema kısıtı dizi tutuyor ve BOŞ LİSTE "kısıt yok" demek. "Tüm lisans
  öğrencileri" diyen bir bursta bölüm alanı boş bırakılmalı — bütün
  bölümleri tek tek yazmak, kısıt olmayan yere kısıt koymak olur.
*/
const ListeAlani: React.FC<{
  etiket: string;
  ipucu: string;
  deger: string[];
  onDegis: (v: string[]) => void;
}> = ({ etiket, ipucu, deger, onDegis }) => (
  <label className="block text-sm font-semibold">
    {etiket}
    <span className="ml-1.5 text-xs font-normal text-gray-500">{ipucu}</span>
    <input
      value={(deger || []).join(', ')}
      onChange={(e) =>
        onDegis(e.target.value.split(',').map((x) => x.trim()).filter(Boolean))
      }
      placeholder="Virgülle ayır — kısıt yoksa boş bırak"
