...
/**
 * Staj yolculuğu haritası.
 *
 * NEDEN HARİTA
 * ------------
 * Rehber merkezi bir başlık listesiydi: on tane beyaz kart, hepsi aynı
 * görünüyor, hangisinden başlanacağı belli değil. Oysa bu başlıkların
 * arasında GERÇEK BİR SIRA var — önce bölümünü tanırsın, sonra CV yazarsın,
 * sonra başvurursun, sonra mülakata girersin.
 *
 * Harita o sırayı gösteriyor. Kişi nerede olduğunu görüyor ve "sıradaki
 * adım ne" sorusunun cevabı kendiliğinden çıkıyor.
 *
 * Mobilde dikey (numaralar alt alta, aralarında çizgi), geniş ekranda yatay.
 * İki düzen de aynı veriden çiziliyor.
 */
export const YolculukHaritasi: React.FC<{
  duraklar: Durak[];
  ...
  <div className="relative">
    ...
    <ol className="sm:hidden space-y-0">
      {duraklar.map((d, i) => (
        /*
          DİKKAT: alt boşluk `li` üzerinde, buton üzerinde değil.

          Önce butona `pb-6 last:pb-0` yazılmıştı. Ama buton kendi `li`sinin
          TEK çocuğu; yani `last:` her butonda eşleşiyor ve `pb-0` kazanıyordu.
          Sonuç: beş durak da sıfır aralıkla, birbirine yapışık çiziliyordu ve
          aralarındaki kesikli çizgiye yer kalmıyordu.
        */
        <li key={d.baslik} className="relative flex gap-4 pb-7 last:pb-0">
          {i < duraklar.length - 1 && (
            <span
              aria-hidden="true"
              className="absolute left-[27px] top-16 bottom-1 w-0.5 border-l-2 border-dashed border-gray-300"
            />
          )}
          <button
            type="button"
          ...
          >
            ...
            <span className="min-w-0 pt-1.5">
              <span className="flex items-center gap-1.5 font-bold text-gray-900">
                {d.baslik}
                <ArrowRight className="w-3.5 h-3.5 text-gray-300 transition-transform group-hover:translate-x-0.5 group-hover:text-blue-500" />
              </span>
              <span className="block text-sm text-gray-500 leading-snug mt-0.5">{d.ozet}</span>
            </span>
          </button>
        </li>
      ))}
        ...
        <React.Fragment key={d.baslik}>
          <li className="flex-1 min-w-0">
            ...
            >
              ...
              <span className="block font-bold text-sm text-gray-900 leading-snug">
                {d.baslik}
              </span>
              <span className="block text-xs text-gray-500 leading-snug">{d.ozet}</span>
            </button>
          </li>
          {/* Duraklar arası bağlantı; sonuncudan sonra çizilmiyor. */}
          {i < duraklar.length - 1 && (
            <li aria-hidden="true" className="shrink-0 pt-8">
              <span className="block w-6 lg:w-10 border-t-2 border-dashed border-gray-300" />
            </li>
          )}
        </React.Fragment>
      ))}
