...
export const SkillQuizzesView: React.FC<SkillQuizzesViewProps> = ({
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 pb-16 animate-in fade-in duration-200">
      {/* Top Banner */}
      <div className="bg-white rounded-2xl p-6 sm:p-7 border border-gray-200 shadow-xs flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        ...
        {/* Global Scorecard */}
        <div className="p-4 bg-gradient-to-br from-emerald-50 to-teal-50 rounded-2xl border border-emerald-200 flex items-center gap-4 shrink-0 shadow-2xs">
          ...
          <div>
            <span className="text-xs font-semibold text-emerald-900">Kazanılan Rozetler</span>
            <p className="text-xl font-black text-emerald-950">
              {totalEarnedCount} <span className="text-sm font-medium text-emerald-700">/ {totalQuizzesCount} Doğrulandı</span>
            </p>
            <div className="w-32 bg-emerald-200/80 rounded-full h-1.5 mt-1.5 overflow-hidden">
              <div
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-white p-3 sm:p-4 rounded-xl border border-gray-200 shadow-2xs">
        {/* Search Bar */}
        <div className="relative w-full sm:w-80">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 text-xs rounded-lg border border-gray-200 bg-white text-gray-900 focus:outline-hidden focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all placeholder:text-gray-400 font-medium"
          />
          {searchQuery && (
            <button
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
          <button
            onClick={expandAll}
            className="px-3 py-1.5 rounded-lg text-xs font-semibold text-gray-600 hover:text-gray-900 bg-gray-100 hover:bg-gray-200 transition-all cursor-pointer"
          >
            Tümünü Aç
          </button>
          <button
          ...
          return (
            ...
            >
              ...
              <button
                ...
                className="w-full p-4 sm:p-5 flex items-center justify-between text-left hover:bg-gray-50/80 transition-colors cursor-pointer select-none"
              >
                <div className="flex items-center gap-3.5 sm:gap-4 min-w-0">
                  {/* Category Themed Icon */}
                  <div
                    className={`w-11 h-11 rounded-xl flex items-center justify-center shrink-0 border ${category.colorScheme.bg} ${category.colorScheme.border}`}
                  >
                ...
                {/* Toggle Chevron */}
                <div className="p-2 rounded-full hover:bg-gray-200/60 text-gray-400 hover:text-gray-700 transition-colors shrink-0 ml-2">
                  {isExpanded ? (
              ...
              {isExpanded && (
                <div className="px-4 sm:px-6 pb-5 pt-2 border-t border-gray-100 bg-gray-50/40">
                      ...
                      {categoryQuizzes.map((quiz) => {
                        ...
                        return (
                          <div
                            ...
                            className={`p-4 rounded-xl border transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white ${
                              isEarned
                                ?'border-emerald-200 shadow-2xs hover:border-emerald-300'
                                :'border-gray-200 shadow-2xs hover:border-blue-300 hover:shadow-xs'
                            }`}
                          >
                            {/* Left: Test Info */}
                            ...
                            {/* Right: Action Button */}
                            <div className="flex items-center sm:justify-end shrink-0 pt-2 sm:pt-0 border-t sm:border-t-0 border-gray-100">
                              <button
