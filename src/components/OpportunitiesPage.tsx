...
export const OpportunitiesPage: React.FC<{ path: string; userId: string | null; student: StudentProfile | null; onNavigate: (path: string) => void; onRequireLogin: () => void }> = ({ path, userId, student, onNavigate, onRequireLogin }) => {
  ...
  return <main className="w-full max-w-[1536px] mx-auto px-4 sm:px-6 lg:px-8 xl:px-10 py-7 pb-24 lg:pb-10 space-y-6">
    <section className="rounded-3xl border border-blue-100 bg-gradient-to-br from-blue-600 to-indigo-700 text-white p-6 sm:p-8 shadow-sm"><p className="text-xs font-bold uppercase tracking-[0.18em] text-blue-100">StajımVar fırsat merkezi</p><h1 className="mt-2 text-2xl sm:text-3xl font-extrabold tracking-tight">{heading}</h1><p className="mt-2 max-w-2xl text-sm sm:text-base text-blue-50">{savedOnly ? 'Sonradan incelemek için kaydettiğin fırsatlar.' : calendar ? 'Yaklaşan son başvuru tarihlerini tek yerde izle.' : matching ? 'Profilindeki doğrulanabilir bilgilerle hesaplanan sonuçlar.' : 'Burs, kredi, eğitim, yurtdışı ve yarışma fırsatlarını resmî kaynaklarıyla tek yerden takip et.'}</p>{!savedOnly&&!calendar&&!matching&&state==='ready'&&<div className="mt-6 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-white/10 p-3"><b className="block text-xl">{overview.openCount}</b><span className="text-xs text-blue-100">Başvurusu devam eden</span></div><div className="rounded-2xl bg-white/10 p-3"><b className="block text-xl">{overview.scholarshipAndCreditCount}</b><span className="text-xs text-blue-100">Burs ve kredi fırsatı</span></div><div className="rounded-2xl bg-white/10 p-3">{overview.nearest && overview.daysLeft != null ? <><b className="block text-sm truncate">{overview.daysLeft} gün kaldı</b><span className="text-xs text-blue-100">{overview.nearest.title}</span></> : <><b className="block text-xl">{items.length}</b><span className="text-xs text-blue-100">Takip ettiğimiz fırsat</span></>}</div></div>}</section>
